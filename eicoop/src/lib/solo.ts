...
export class SoloStatus {
  ...
  constructor(contract: ei.ILocalContract, backup: ei.IBackup) {
    ...
    this.expirationTime = dayjs(contract.timeAccepted! * 1000).add(this.contract.lengthSeconds!, 'seconds');
    this.secondsRemaining = this.expirationTime.diff(dayjs(), 'seconds', true);
    const hoursLeftAtRefresh = Math.max(this.expirationTime.diff(this.refreshTime, 'hours', true), 0);
    ...
    this.projectedHourlyLayingRateUncappedAtFullHabs = farm.layableEggsPerChickenPerHour * farm.habSpace;
    ...
    this.boosts = (farm.farm.activeBoosts ?? []).filter(boost => !!boost.boostId && (boost.timeRemaining ?? 0) > 0);
  ...
  get estimatedLeagueStatusNow(): ContractLeagueStatus {
    ...
    const estimatedEggsLaid = this.eggsLaid + Math.min(hoursLeftAtRefresh, hoursSinceRefresh) * this.eggsPerHour;
