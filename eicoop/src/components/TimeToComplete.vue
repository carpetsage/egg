...
<template>
  <tippy class="text-gray-900 dark:text-gray-100">
    <span :class="completionStatusFgColorClass(completionStatus)">{{
      formatDuration(expectedTimeToComplete)
    }}</span>
    expected
    <template v-if="expectedTimeToComplete > 0" #content>
      <p>Expected to complete at
      <span :class="completionStatusFgColorClass(completionStatus)">
        {{ expectedFinalCompletionDate.format('YYYY-MM-DD HH:mm') }}
      </span>.</p>
      <template v-if="grade">
        <p>Predicted total time take for contract completion:
        <span :class="completionStatusFgColorClass(completionStatus)">
...
export default defineComponent({
  ...
  setup(props) {
    const {offline, completionStatus, expectedFinalCompletionDate, expectedTimeToComplete, status} = toRefs(props)
