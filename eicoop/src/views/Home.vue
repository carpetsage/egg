    ...
    <base-collapsible-panel title="Notes and instructions" :collapse="!showInstructions" :toggle="toggleInstructions">
      <ul class="list-disc pt-2 pl-5 space-y-1">
        <li class="text-green-700 dark:text-green-300">
          Bring up the coop selector by clicking on a contract title or ID in the table below. Alternatively, click on
          the
          <svg viewBox="0 0 20 20" fill="currentColor" class="inline h-4 w-4 text-green-700 dark:text-green-300">
            <path
              fill-rule="evenodd"
          ...
          button in the navigation bar or use the hotkey "c" to bring up the coop selector on any page.
        </li>

        <li class="text-green-700 dark:text-green-300">Active contracts are marked in green.</li>

        <li class="text-green-700 dark:text-green-300">
          When a new contract is released, it may take some time before it is added to the master list. In the mean
          time, you can check associated coop statuses by manually entering the contract ID. Note that the contract ID
          is not visible within the game. If you're on the
          <a href="https://discord.gg/egginc" target="_blank" class="text-green-700 dark:text-green-300 underline"
        ...
        <li class="text-green-700 dark:text-green-300">
          Goals and rewards for each contract can be expanded either through the expand button on the left of each row,
          or through the "Expand all" button.
        </li>

        <li class="text-green-700 dark:text-green-300">
          Each column in the contract list can be used as the sorting criterion by clicking on the sort button in the
          column header (click once for ascending order, twice for descending, and thrice to return to default). Note
          that several columns can also be filtered by clicking on the filter button in the column header.
        </li>

        <li class="text-green-700 dark:text-green-300">
          Contract offering dates are derived from contract expiration dates with a simple heuristic: original contracts
          are up for three weeks, whereas leggacies are up for one. This is not always true for early contracts in 2018
          and part of 2019, so offering dates for early contracts may not be accurate.
        </li>

        <li class="text-green-700 dark:text-green-300">
          Due to architectural tradeoffs, the contract selection menu in the coop selector only lists the latest
          incarnation of each contract, so you may be presented a leggacy when trying to select the original. Rest
          assured this is only a display quirk.
        </li>
      </ul>
...
import { getLocalStorage, rawContractListHash, setLocalStorage } from '@/lib';
...
export default defineComponent({
  ...
  setup() {
    ...
    const contracts = computed(() => [...contractsStore.list].reverse());
