...
const supportedMathSymbols = new Map(
  (
    Object.getOwnPropertyNames(Math)
      ...
      .map(symbol => [symbol.toLowerCase(), `Math.${symbol}`]) as [string, string][]
  ).concat(
...
export function calculateWithOoMUnits(expr: string): number | null {
  ...
  expr = expr.replaceAll(mathSymbolRegExpGlobal, value => supportedMathSymbols.get(value.toLowerCase())!);
