import { decodeMessage, ei } from "lib";
...
export const rawContractList: ContractProps[] = (() => {
  ...
  for (const contract of list) {
    if (count.has(contract.identifier!)) {
      // Leggacy
      contract.offeringTimestamp = contract.expirationTime! -
        LEGGACY_CONTRACT_VALID_DURATION;
      count.set(contract.identifier!, count.get(contract.identifier!)! + 1);
    } else {
      // Original
      contract.offeringTimestamp = contract.expirationTime! -
        ORIGINAL_CONTRACT_VALID_DURATION;
...
export function getUserContractList(backup: ei.IBackup): UserContract[] {
  const localContracts: ei.ILocalContract[] =
    (backup.contracts?.contracts || []).concat(
...
function newUserContract(
  ...
  const contribution =
    (isCoop
      ? contract.coopLastUploadedContribution
  ...
  let goals = contract.grade
    ? props.gradeSpecs![contract.grade - 1].goals
    : props.goals;
