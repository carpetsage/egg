...
if [ -s loot_raw.json ]; then
  ...
  # first: a redirect truncates src/lib/loot.json before jq ever sees the input, so an
  # invalid response would otherwise leave an empty file behind and delete the evidence.
  script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
  tmp=$(mktemp "${script_dir}/src/lib/loot.json.XXXXXX")
  if jq -r . loot_raw.json > "$tmp"; then
    mv "$tmp" "${script_dir}/src/lib/loot.json"
    rm loot_raw.json
  else
    rm -f "$tmp"
    exit 1
