<template>
  ...
  <div v-for="m in t1LootDataByMission.slice(0, 10)" :key="m.missionId + m.target">
    ...
    <div v-if="!m.notEnoughData || config.showNodata">
      <mission-name :mission="m.mission" :target="m.target" />
    </div>

    <craft-rate
      :target="artifactProperties"
      :mission="m.mission"
      :level="m.mission.maxLevel"
      :total-crafts="m.expectedCrafts"
      :applicable-drops="m.applicableItems"
      :total-ships="m.sentShips"
      :not-enough-data="m.notEnoughData"
      :time-limited="m.timeLimited"
      :fuel-limited-ships="m.maxFuelShips"
      :hide-when-not-enough="!config.showNodata"
    />
  </div>
</template>
...
import CraftRate from '@/components/CraftRate.vue';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const isArtifact = computed(() => artifactProperties.value.afx_type === ei.ArtifactSpec.Type.ARTIFACT);
    ...
    const recursiveIngredients = computed(() => {
      ...
      const t1Ingredients = expandArtifactIngredients(artifactProperties.value);
    ...
    const t1LootDataByMission = computed(() => {
      ...
      for (const mission of missions) {
        if (!config.value.shipVisibility[mission.shipType]) {
          continue;
        }
        try {
          ...
          let maximumNumberOfShipsSent = Math.floor(
            (3.0 * maxWaitTimeSeconds.value) / mission.boostedDurationSeconds(config.value)
          );
          ...
          for (const target of maxLevelMissionData.targets) {
            ...
            for (const item of targettedMissionItems) {
              ...
              const expectedDrops = Math.floor(
                (sum(item.counts) / target.totalDrops) * missionCapacity * maximumNumberOfShipsSent
              );

              const itemData = getArtifactTierPropsFromId(item.itemId);
              expectedApplicableItems.push({
                icon_filename: itemData.icon_filename,
                count: expectedDrops,
              } as Ingredient);
            ...
            for (const [component, requiredCount] of localRecursiveIngredients.tier1s.entries()) {
              if (expectedT1Counts.has(component)) {
                minCrafts = Math.min(minCrafts, expectedT1Counts.get(component) / requiredCount);
              } else {
                minCrafts = 0;
              }
            }
            ...
            data.push({
              ...
              // Wait to "floor" this metric to keep comparability of ship list
              expectedCrafts: minCrafts, //Math.floor(minCrafts),
...
function expandArtifactIngredients(
  item: Tier,
  applicableItems: Set<string> | undefined = undefined,
  expansionMultiplier: number = 1
): Map<string, number> {
  const items = new Map();

  if (
    item.afx_level === ei.ArtifactSpec.Level.INFERIOR &&
    (applicableItems === undefined || applicableItems.has(item.id))
  ) {
  ...
  for (const ingredient of item.recipe.ingredients) {
    ...
    const recursiveMap = expandArtifactIngredients(
      getArtifactTierPropsFromId(ingredient.id),
      applicableItems,
      expansionMultiplier * ingredient.count
    );

    for (const [key, value] of recursiveMap.entries()) {
      if (items.has(key)) {
