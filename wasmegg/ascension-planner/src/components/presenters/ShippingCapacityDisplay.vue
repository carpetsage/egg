    ...
    <div class="bg-slate-50/50 rounded-2xl p-6 border border-slate-100 shadow-inner flex justify-between items-center">
      <div>
        <div class="flex items-center gap-3 mb-1">
          ...
          <div class="badge-premium badge-brand py-0 text-[8px]">{{ activeVehicleCount }}/{{ output.maxVehicleSlots }} Vehicles</div>
        </div>
        <div class="text-3xl font-bold text-slate-800 tracking-tight">
          {{ formatNumber(convertedCapacity, 1) }} <span class="text-sm font-medium text-slate-400">/{{ timeUnitLabel }}</span>
        </div>
      </div>
      <!-- Time Unit Toggle -->
    ...
    <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
      ...
      <div class="divide-y divide-slate-50">
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center border border-slate-100 shadow-inner group-hover:scale-110 transition-transform">
              <img :src="iconURL(getColleggtibleIconPath('bust_unions'), 64)" class="w-5 h-5 object-contain" alt="Bust Unions" />
            </div>
            <div>
              <div class="text-[11px] font-bold text-slate-700">Bust Unions</div>
            </div>
          </div>
          <span class="font-mono-premium text-sm font-bold" :class="bustUnionsLevel > 0 ? 'text-slate-900' : 'text-slate-300'">
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center border border-slate-100 shadow-inner group-hover:scale-110 transition-transform">
              <img :src="iconURL(getColleggtibleIconPath('lithium'), 64)" class="w-5 h-5 object-contain" alt="Lithium" />
            </div>
            <div>
              <div class="text-[11px] font-bold text-slate-700">Lithium</div>
            </div>
          </div>
          <span class="font-mono-premium text-sm font-bold" :class="lithiumMultiplier < 1 ? 'text-slate-900' : 'text-slate-300'">
        ...
        <div class="px-5 py-3 flex justify-between items-center bg-slate-50/80">
          <span class="text-[10px] font-black text-slate-600 uppercase tracking-widest">Total Vehicle Cost Multiplier</span>
        ...
        >
          <div class="flex items-center gap-4">
            ...
            <div v-if="slot.vehicleId !== null" class="flex items-center gap-3 flex-1">
              <div class="w-14 h-10 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 shadow-inner group-hover:scale-105 transition-transform overflow-hidden">
                <img
              ...
              <div>
                <div class="text-[11px] font-bold text-slate-700 leading-tight">{{ getVehicleType(slot.vehicleId)?.name }}</div>
                <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                   {{ slot.vehicleId === 11 ? `${slot.trainLength} cars` : '' }}
                </div>
              </div>
            </div>
            <span v-else class="text-[11px] font-bold text-slate-300 italic uppercase tracking-wider flex-1">Slots</span>

            <!-- Pricing / Capacity -->
            <div class="text-right">
              <div v-if="slot.vehicleId !== null" class="font-mono-premium text-xs font-bold text-slate-700">
                {{ formatNumber(getVehicleCapacity(index), 1) }}<span class="text-[10px] text-slate-400 ml-0.5">/s</span>
              </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
      ...
      <div class="divide-y divide-slate-50">
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span class="font-mono-premium text-sm font-bold text-slate-700">{{ formatMultiplier(output.universalMultiplier) }}</span>
        </div>
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Hover Efficiency</span>
          <span class="font-mono-premium text-sm font-bold text-slate-700" :class="output.hoverMultiplier !== 1 ? 'text-slate-900' : 'text-slate-300'">
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span class="font-mono-premium text-sm font-bold text-slate-700" :class="output.hyperloopMultiplier !== 1 ? 'text-slate-900' : 'text-slate-300'">
            {{ formatMultiplier(output.hyperloopMultiplier) }}
          </span>
        </div>
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          <div class="flex items-center gap-2">
            <img :src="iconURL(getColleggtibleIconPath('transportation_lobbyist'), 64)" class="w-4 h-4 object-contain opacity-60" alt="Transportation Lobbyists" />
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span class="font-mono-premium text-sm font-bold" :class="output.colleggtibleMultiplier !== 1 ? 'text-slate-900' : 'text-slate-300'">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                ...
                <span v-if="research.hoverOnly" class="badge-premium py-0 text-[7px] bg-slate-100 text-slate-400">Hover Only</span>
                <span v-if="research.hyperloopOnly" class="badge-premium py-0 text-[7px] bg-slate-100 text-slate-400 border-brand-primary/20 text-slate-900">Hyperloop Only</span>
              </div>
            </div>
          </div>
          <div class="text-right">
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Max {{ maxTrainLength }} cars/train</div>
            </div>
          </div>
          <div class="text-right">
            <div class="font-mono-premium text-xs font-bold text-slate-700">{{ gravitonCouplingLevel }} <span class="text-slate-300 font-normal">/</span> 5</div>
          </div>
        </div>
          ...
          <div class="p-3 bg-white rounded-xl border border-slate-100 shadow-sm flex justify-between items-center group hover:shadow-md transition-shadow">
            <div class="flex items-center gap-3">
               <div class="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center border border-slate-100 shadow-inner overflow-hidden">
                <img :src="iconURL(getColleggtibleIconPath('transportation_lobbyist'), 64)" class="w-5 h-5 object-contain" alt="Transportation Lobbyists" />
              </div>
              <div>
                <div class="text-[11px] font-bold text-slate-700 leading-tight">Transportation Lobbyists</div>
              </div>
            </div>
            <span class="font-mono-premium text-xs font-bold text-slate-700">{{ transportationLobbyistLevel }} <span class="text-slate-300">/</span> 30</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
      ...
      <div class="px-5 py-4 space-y-4">
        ...
        <div class="flex justify-between items-center p-3 bg-slate-50/50 rounded-xl border border-slate-100">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-white flex items-center justify-center border border-slate-100 shadow-sm">
              <img :src="iconURL(getColleggtibleIconPath('carbon-fiber'), 64)" class="w-5 h-5 object-contain" alt="Carbon Fiber" />
            </div>
            <div>
              <div class="text-[11px] font-bold text-slate-700">Carbon Fiber</div>
              <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ formatTier(colleggtibleTiers.carbonFiber) }}</div>
            </div>
          </div>
          <span class="font-mono-premium text-sm font-bold text-slate-900">
        ...
        <div class="flex justify-between items-center p-3 bg-slate-50/50 rounded-xl border border-slate-100">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-white flex items-center justify-center border border-slate-100 shadow-sm">
              <img :src="iconURL(getColleggtibleIconPath('pumpkin'), 64)" class="w-5 h-5 object-contain" alt="Pumpkin" />
            </div>
            <div>
              <div class="text-[11px] font-bold text-slate-700">Pumpkin</div>
              <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ formatTier(colleggtibleTiers.pumpkin) }}</div>
            </div>
          </div>
          <span class="font-mono-premium text-sm font-bold text-slate-900">
        ...
        <p class="text-[9px] text-slate-400 uppercase font-black tracking-widest px-1">Global logistics efficiency from collection achievements</p>
      </div>
    </div>
  </div>
</template>
