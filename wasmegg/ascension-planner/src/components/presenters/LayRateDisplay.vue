      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
          ...
          <div class="text-2xl font-bold text-slate-800 tracking-tight">
            {{ formatNumber(output.ratePerChicken, 3)
            }}<span class="text-xs text-slate-400 ml-1">/{{ timeUnitLabel }}</span>
          </div>
        </div>
        <div class="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
      ...
      <div class="divide-y divide-slate-50">
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span class="font-mono-premium text-sm font-bold text-slate-700"
            >{{ formatNumber(output.baseRatePerSecond * 60, 4) }}/min</span
          >
        </div>
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          <div class="flex items-center gap-2">
            <img
              :src="iconURL(getColleggtibleIconPath('silicon'), 64)"
              class="w-4 h-4 object-contain opacity-60"
              alt="Silicon"
            />
          ...
          <span
            class="font-mono-premium text-sm font-bold"
            :class="output.siliconMultiplier !== 1 ? 'text-slate-900' : 'text-slate-300'"
          >
        ...
        <div class="px-5 py-4">
          ...
          >
            ...
            <span class="font-mono-premium text-xs font-bold text-slate-700"
              >{{ epicComfyNestsLevel }} <span class="text-slate-300">/</span> 20</span
            >
          </div>
        </div>
      </div>
