    ...
    <div class="bg-slate-50/50 rounded-2xl p-6 border border-slate-100 shadow-inner flex justify-between items-center">
      <div>
        ...
        <div class="text-3xl font-bold text-slate-800 tracking-tight">
          {{ formatFullNumber(output.totalFinalCapacity) }}
          <span class="text-sm font-medium text-slate-400">chickens</span>
        </div>
      </div>
      ...
      <div class="divide-y divide-slate-50">
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span
            ...
            :class="cheaperContractorsLevel > 0 ? 'text-slate-900' : 'text-slate-300'"
          >
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          <div class="flex items-center gap-3">
            ...
            >
              <img
                :src="iconURL(getColleggtibleIconPath('flame-retardant'), 64)"
                class="w-5 h-5 object-contain"
                alt="Flame Retardant"
              />
            </div>
          ...
          <span
            class="font-mono-premium text-sm font-bold"
            :class="flameRetardantMultiplier < 1 ? 'text-slate-900' : 'text-slate-300'"
          >
        ...
        >
          <div class="flex items-center gap-4">
            ...
            <div v-if="hab.habId !== null" class="flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 shadow-inner group-hover:scale-105 transition-transform"
              >
                <img
              ...
              <div>
                <div class="text-[11px] font-bold text-slate-700 leading-tight">
                  {{ getHabById(hab.habId as HabId)?.name }}
                </div>
          ...
          <div class="text-right">
            ...
            <div
              v-if="hab.habId !== null"
              class="text-[10px] font-black text-slate-400 uppercase tracking-widest opacity-60"
            >
      ...
      <div class="divide-y divide-slate-50">
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span class="font-mono-premium text-sm font-bold text-slate-700">{{
            formatNumber(output.totalBaseCapacity, 0)
          }}</span>
        </div>
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span
            ...
            :class="output.portalResearchMultiplier !== 1 ? 'text-slate-900' : 'text-slate-300'"
          >
        ...
        <div class="px-5 py-3 flex justify-between items-center group hover:bg-slate-50 transition-colors">
          ...
          <span
            class="font-mono-premium text-sm font-bold"
            :class="output.peggMultiplier !== 1 ? 'text-slate-900' : 'text-slate-300'"
          >
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                ...
                <span v-if="research.portalOnly" class="badge-premium py-0 text-[7px] bg-slate-100 text-slate-400"
                  >Portal Only</span
                >
              </div>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
      ...
      <div class="px-5 py-4">
        ...
        <p class="text-[9px] text-slate-400 uppercase font-black tracking-widest mt-3 px-1">
          Hab capacity bonus from collection achievements
        </p>
      </div>
    </div>
  </div>
...
const purchasedCount = computed(() => props.output.habBreakdown.filter(h => h.habId !== null).length);
...
const currentHabIds = computed(() => props.output.habBreakdown.map(h => h.habId));
