...
export interface CloudPlanEntry {
  ...
  lastAccessed?: number;
  /** Whether the user has opted this plan into cloud sync. */
  syncEnabled?: boolean;
}
...
export class CloudSyncService {
  ...
  private async checkOk(response: Response, context: string): Promise<void> {
    if (!response.ok) {
      ...
      throw new CloudSyncError(`${context}: HTTP ${response.status}${body ? ` — ${body}` : ''}`, response.status);
    }
  }
  ...
  async uploadPlan(
    ...
    if (syncEnabled !== undefined) {
      headers['X-Plan-Sync-Enabled'] = String(syncEnabled);
    }
 ...
 */
export function createCloudSyncService(opts: { eid: string; eidHash: string }): CloudSyncService | null {
