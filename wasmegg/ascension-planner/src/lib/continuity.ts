...
export async function checkAndHandleContinuity(
    actions: Action[],
    newAction: Omit<Action, 'id' | 'index' | 'timestamp' | 'endState' | 'cost' | 'dependents' | 'totalTimeSeconds'> & { dependsOn: string[] },
    insertIndex: number,
    ...
    for (const action of futureActions) {
        ...
        // 2. Hab (Kindness/Integrity)
            ...
            if (
                existingPayload.slotIndex === newPayload.slotIndex &&
                newPayload.habId >= existingPayload.habId
            ) {
                conflicts.push(action);
            }
        }
        ...
        // 3. Vehicle (Kindness/Integrity)
            ...
            if (
                existingPayload.slotIndex === newPayload.slotIndex &&
                newPayload.vehicleId >= existingPayload.vehicleId
            ) {
        ...
        // 4. Silo (Resilience)
            ...
            newAction.type === 'buy_silo' &&
            action.type === 'buy_silo' &&
            currentEgg === 'resilience'
        ) {
