...
<template>
  ...
  <template v-else>
    ...
    <div v-if="multiplier !== undefined" class="text-center text-xs my-2">
      <div>
        <template v-if="!mirroring">Virtual e</template><template v-else>E</template>arnings multiplier from artifacts:
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const mirroring = computed(() => (strategy.value ?? Infinity) <= PrestigeStrategy.PRO_PERMIT_LUNAR_MIRROR);
    return {
      empty,
      alreadyOptimal,
      multiplier,
