<template>
  <template v-if="unresolvedCount > 0">
    <div class="rounded-md bg-red-100 text-red-800 text-sm w-full px-4 py-2 mx-auto mb-4 shadow">
      <strong>Contract data is currently unavailable.</strong>
      Due to a recent Egg, Inc. update, the game no longer provides contract details needed by this tool. Details for
      {{ unresolvedCount }} contract{{ unresolvedCount !== 1 ? 's are' : ' is' }}
      missing. A fix is in progress — thank you for your patience!
    </div>
  </template>
  <template v-else>
    ...
    <div class="mx-4 my-4 xl:mx-0 text-xs">
      ...
      <ul class="list-disc">
        <li>
          For contracts with multiple incarnations, i.e. original run and leggacy run(s), only one incarnation is
          listed. If the player has attempted the contract, the last attempted incarnation is shown; otherwise, the
          latest incarnation is shown.
        </li>
        <li>
        ...
        <li>
          ...
          prophecy eggs is noted in parentheses if it's more than 1). The column is blank if there's no PE associated
          with the contract. Otherwise, for older contracts without standard/elite tiers, this column should look like
          "#2", meaning the second reward being a PE; for newer contracts with tiers, this column should look like "std
          #3", meaning the third reward of standard tier being a PE, or "elt #2", meaning the second reward of elite
          tier being a PE. The tier shown is the tier the player last attempted the contract on, with the exception that
          if the player completed none of the goals then the tier shown defaults to elite (since in that case it's
          harder to tell which tier the player was on at the time, if they did make an attempt).
        </li>
        <li>
